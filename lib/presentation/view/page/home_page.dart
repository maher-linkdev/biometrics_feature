...
class HomePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Biometrics Authentication')),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Text('Biometrics Status:', style: Theme.of(context).textTheme.headlineSmall),
            const SizedBox(height: 16),
            Text('Available: ${biometricsState.isAvailable}'),
            Text('Enabled: ${biometricsState.isEnabled}'),
            Text('Available Types: ${biometricsState.availableTypes.join(", ")}', textAlign: TextAlign.center),
            const SizedBox(height: 32),
            ElevatedButton(
              onPressed:
                  biometricsState.isAvailable && biometricsState.isEnabled
